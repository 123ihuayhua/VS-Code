...
def main(page: Page):
    ...
    tel = TextField(label="Teléfono")

    # Editar
    edit_codtxt = TextField(label="Código", disabled=True)
    edit_dnittxt = TextField(label="DNI")  
    edit_apepattxt = TextField(label="Apellido Paterno")  
    edit_apemattxt = TextField(label="Apellido Materno")  
    edit_nomttxt = TextField(label="Nombre")
...
# ---------
    ...
    def createbtn(e):
        ...
        edit_dnittxt.value = e.control.data['HueDNI']
        edit_apepattxt.value = e.control.data['HueApePat']
        edit_apemattxt.value = e.control.data['HueApeMat']
        edit_nomttxt.value = e.control.data['HueNom']
    ...
    #Iconos
    def load_icons(row):
            if (row['HueEstReg'] == 'I' or row['HueEstReg'] == '*'):
    ...
    def addtodb(e):
        try:
            sql = "INSERT INTO HUESPED (HueCod, HueDNI, HueApePat, HueApeMat, HueNom, HuerTel) VALUES (%s, %s, %s, %s, %s, %s)"
            val = (cod.value, dni.value, apepat.value, apemat.value, nom.value, tel.value,)
            cursor.execute(sql, val)
            mydb.commit()
    ...
    page.add(
        AppBar(
            title = Text("TABLA HUESPED", size = 30),
            bgcolor = colors.TEAL,
